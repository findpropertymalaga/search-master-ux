...
const PropertyListings = () => {
  ...
  // Back to working property hook with speed optimizations
  const { 
    properties, 
    allProperties, 
    loading, 
    ...
    detectedMobile,
    page
  } = usePropertyData(currentFilters, isInitialized);
  ...
  useEffect(() => {
    console.log(`=== PropertyListings OPTIMIZED ===`);
    console.log(`Properties: ${properties.length}, Total: ${totalCount}, Loading: ${loading}, Initializing: ${isInitializing}`);
  }, [properties.length, totalCount, loading, isInitializing]);
  ...
  useEffect(() => {
    if (properties.length > 0 && totalCount > 0) {
      ...
      // Store search context for enhanced navigation
      storeSearchContext(currentFilters, sortOrder, totalCount, false);
  ...
  useEffect(() => {
    if (properties.length > 0 && page > 1) {
      const storageKey = 'scrollMemory_buy';
